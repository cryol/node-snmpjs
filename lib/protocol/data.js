...
canonicalizeOID(oid)
{
	...
	for (i = 0; i < addr.length; i++) {
		...
		if (n < 0) {
			console.log(addr, n, i, addr[i], canon, oid)
...
SnmpOctetString(value)
{
	...
	this.__defineSetter__('value', function (v) {
		if (typeof (v) === 'object' && (v instanceof ASN1.Reader)) {
			...
			v = new Buffer.from(b.length);
			b.copy(v);
			self._value = b;
		} else if (Buffer.isBuffer(v)) {
			self._value = new Buffer.from(v.length);
			v.copy(self._value);
		} else if (typeof (v) === 'string') {
			self._value = new Buffer.from(v, 'ascii');
		} else {
			throw new TypeError('value is of incompatible type');
		}
	});
